<div class="Admin-User-Manager">
    <div class="Noitifications">
        ...
        if (isset($_SESSION['admin_u'])) {
            $Role_Login = $_SESSION['admin_u']['Role'];
        } else if (isset($_SESSION['admin_a'])) {
            $Role_Login = $_SESSION['admin_a']['Role'];
        } else {
            $Role_Login = null;
        }
            ...
            <table border="1">
                ...
                foreach ($List_Users as $user) {
                    $List_U .= '<tr>
                            ...
                            <td>' . $user['Name'] . '</td>
                            <td>' . $user['Email'] . '</td>';
                    $List_U .= $user['Status'] == 0 ? '<td> Active </td>' : '<td> Deactive </td>';
                    if ($user['Role'] == 0) {
                        $List_U .= '<td> Admin </td>';
                    } else if ($user['Role'] == 1) {
                    ...
                    if ($Role_Login == 0) {
                        $List_U .= '
                            <td><a href="admin.php?Admin_Action=users&User_Action=edit&ID=' . $user['ID'] . '">Edit</a> | <a href="admin.php?Admin_Action=users&User_Action=delete&ID=' . $user['ID'] . '">Delete</a></td>
                        ';
                    }
                    $List_U .= '</tr>';
                }
