...
if (isset($_POST['submit'])) {
    ...
    foreach ($result as $col => $row) {
        if ($row['Username'] == $username && $row['Passwords'] == sha1($password) && ($row['Role'] == 0 || $row['Role'] == 1) && $row['Status'] == 1) {
            header('Location: admin.php?Error=Tài Khoản Bị Vô Hiệu Hóa'.$return_admin_login);
            exit;
        } else if ($row['Username'] == $username && $row['Passwords'] == sha1($password) && ($row['Role'] == 0 || $row['Role'] == 1)) {
