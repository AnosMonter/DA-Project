        ...
        if (isset($_POST['Edit_Product'])) {
            ...
            if (empty($_POST['Name'])) {
                ...
                if ($Image_Check == 1) {
                    ...
                    $New_Name_Image = 'Products' . $Name . pathinfo($Image_Name, PATHINFO_EXTENSION);
                    if (move_uploaded_file($Image_Temp, $File_Dir . $New_Name_Image) == false) {
...
function upload_multiple_files($input_name, $upload_dir) {
    ...
    foreach($_FILES[$input_name]['name'] as $key => $name) {
        ...
        if ($error === 0) {
            ...
            // Tạo tên file mới theo dạng số thứ tự
            $new_filename = $file_count . '.' . $file_ext;
            $target_file = $upload_dir . '/' . $new_filename;

            // Tăng biến đếm cho file tiếp theo
            $file_count++;

            // Di chuyển file từ thư mục tạm đến thư mục đích
            if (move_uploaded_file($tmp_name, $target_file)) {
                // File đã được upload thành công
                echo "File $name đã được upload thành công với tên mới: $new_filename";
            } else {
                echo "Có lỗi xảy ra khi upload file $name.";
            }
        } else {
            echo "Lỗi upload file $name: " . $error;
        }
    }
    return true;
}