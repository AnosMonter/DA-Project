            ...
            if (empty($User_List_Products)) {
                ...
                foreach ($User_List_Products as $Val) {
                    $List_Table_Products .= '<tr>
                        <td><a href="index.php?action=products&product-action=detail&id=' . $Val['ID'] . '"><img width="150px" height="100px" src="public/img/Products/' . $Val['Image'] . '" alt="">' . $Val['Name'] . '<a></td>
                        <td>' . number_format($Val['Price'], 0, ',', '.') . ' VNĐ</td>
                        <td><input type="number" name="Quantity-Detail" value="' . $Val['Quantity'] . '" min="0" max="100"></td>
                        <td>' . $Val['Sale'] . ' %</td>
